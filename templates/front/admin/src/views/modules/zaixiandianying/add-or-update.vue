<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.dianyingshipin" label="电影视频" prop="dianyingshipin">
          <file-upload
          tip="点击上传电影视频"
          action="file/upload"
          :limit="1"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.dianyingshipin" label="电影视频" prop="dianyingshipin">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.dianyingshipin)">预览</el-button>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="电影描述" prop="dianyingmiaoshu">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.dianyingmiaoshu" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.dianyingmiaoshu" label="电影描述" prop="dianyingmiaoshu">
                    <span v-html="ruleForm.dianyingmiaoshu"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.dianyingmiaoshu = this.ruleForm.dianyingmiaoshu.replace(reg,'../../../djangouops9/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.dianyingshipin!=null) {
		this.ruleForm.dianyingshipin = this.ruleForm.dianyingshipin.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zaixiandianying/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zaixiandianyingCrossAddOrUpdateFlag = false;
						 this.parent.search();
	...
	this.ruleForm.dianyingshipin = fileUrls;
	this.addEditUploadStyleChange()
    },
